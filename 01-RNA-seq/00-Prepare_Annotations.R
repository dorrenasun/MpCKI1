...
main<-function(){
  ...
  {#Retrieve nomenclatures
    ...
    # #Load nomenclature from Kawamura&Yoshitake
    # MpTF<-fread(paste0(path_annot,"TF_List-Kawamura2022.tsv"),check.names = T)
    # #Check if all ids are from v6.1
    # list.miss<-setdiff(MpTF$gene_id,ids.v6$gene)
    # if (length(list.miss)>0) message("Some TF ids not found in ver6.1!")
    
    #Combine the nomenclatures
    # Mpnames.comb<-rbind(Mpnames.valid[!MpTak_v6.1=="-",.(MpTak_v6.1,Name,product)],
    # MpTF[,.(MpTak_v6.1=gene_id,Name=nomenclature,product)])
    #Sort out duplicates
    ...
    # Mpnames.uniq[,isTF:=(MpTak_v6.1 %in% MpTF$gene_id)]
    # Mpnames.uniq<-merge(data.table(gene=geneID.Tak1),Mpnames.uniq,by="gene",all.x=T)
  }
